...
from app.indicators.volatility import VolatilityCalculator
...
class PriceMonitor:
    ...
    def __init__(self):
        ...
        self.symbols = settings.binance_symbols or ["BTCUSDT", "ETHUSDT", "SOLUSDT"]
        # Binance connector
        ...
        # Chainlink historical connector
        self.chainlink_historical = ChainlinkHistoricalConnector()

        # Chainlink price collector (локальный сборщик цен)
        self.chainlink_collector = ChainlinkPriceCollector()

        # Polymarket RTDS connector
    ...
    async def start(self) -> None:
        ...
        try:
            # Загружаем исторические свечи из Binance
            logger.info("Загрузка исторических свечей из Binance...")
            for symbol in self.symbols:
            ...
            # Ждем получения первых цен от RTDS (максимум 15 секунд)
            logger.info("⏳ Ожидание первых цен от Polymarket RTDS...")
            await self._wait_for_first_prices(timeout=15)
            ...
            logger.info("PriceMonitor запущен (индикаторы: Binance, текущие цены: Polymarket RTDS)")
        except Exception as exc:  # noqa: BLE001
            logger.error("Ошибка при запуске PriceMonitor: %s", exc)
            raise
    
    async def _wait_for_first_prices(self, timeout: int = 15) -> None:
        """
        Ждать получения первых цен от Polymarket RTDS.
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            # Проверяем, получены ли цены для всех символов
            prices_received = sum(
                1 for symbol in symbols if self.polymarket.get_price(symbol) is not None
            )

            if prices_received == len(symbols):
                logger.info(
                    f"✅ Получены первые цены от RTDS для всех {len(symbols)} символов "
                    f"за {asyncio.get_event_loop().time() - start_time:.1f}с"
                )
                return

            if prices_received > 0:
                logger.debug(f"⏳ Получено {prices_received}/{len(symbols)} цен от RTDS...")

            await asyncio.sleep(0.5)

        # Таймаут - логируем предупреждение
        prices_received = sum(
            1 for symbol in symbols if self.polymarket.get_price(symbol) is not None
        )
        logger.warning(
            f"⚠️ Таймаут ожидания RTDS ({timeout}с): получено {prices_received}/{len(symbols)} цен. "
            "Будет использован Binance fallback."
        )
    ...
    async def stop(self) -> None:
        ...
        # Останавливаем Polymarket connector
            ...
            try:
                ...
                for symbol in self.symbols:
                    ...
                    if stats:
                        ...
                        # Логируем метрики (только если не lite режим)
                        if not hasattr(settings, 'lite_logs') or not settings.lite_logs:
                ...
                if (
                    ...
                    for symbol in self.symbols:
                        ...
                        if signal:
                            ...
                            logger.debug("CURRENT SIGNAL: %s → NONE", symbol.upper())
            except Exception as exc:  # noqa: BLE001
    ...
    def _log_stats(self, symbol: str, stats: Dict) -> None:
        ...
        vol_str = (
            " | ".join(
                ...
                for tf, val in volatility.items()
            )
            if volatility
            else "N/A"
        )
        ...
        if should_log:
            ...
            logger.info(
                "%s: $%.2f | Vol: %s | Momentums: %s | RSI: %s | MACD: {%s} | ATR: %s",
                symbol,
                price,
                vol_str,
                momentum_str,
    ...
    def _calculate_indicators(self, symbol: str) -> Dict:
        ...
        # Используем Polymarket RTDS (Chainlink) для текущей цены
    ...
    def _calculate_volatility(self, symbol: str) -> dict[str, float | None]:
        """
        Рассчитать волатильность для символа по разным таймфреймам.
        ...
        значения — волатильность в процентах или None.
        """
        buffer = self.buffers.get(symbol)
        if not buffer:
            return {}
        ...
        for tf in VOLATILITY_TIMEFRAMES:
            ...
            if tf == "1m":
                ...
                df_tf = MultiTimeframeAnalyzer.resample_to_5min(df_1m)
            elif tf == "15m":
                df_tf = MultiTimeframeAnalyzer.resample_to_15min(df_1m)
            elif tf == "1h":
                df_tf = MultiTimeframeAnalyzer.resample_to_hourly(df_1m)
            else:
                continue

            vol = VolatilityCalculator.calculate_for_df(
                df_tf,
                period=VOLATILITY_PERIOD,
                ewm_alpha=VOLATILITY_EWM_ALPHA,
            )
    ...
    def get_volatility(self, symbol: str) -> Optional[dict[str, float | None]]:
        """
        Получить текущую волатильность для символа по разным таймфреймам.
        ...
        if not stats:
            return None
        return stats.get("volatility")
    
    def get_price(self, symbol: str) -> Optional[float]:
        """
        Получить последнюю цену для символа из Polymarket RTDS (Chainlink данные).
        ...
        """
        # Используем Polymarket RTDS WebSocket (Chainlink данные в реальном времени)
        polymarket_price = self.polymarket.get_price(symbol)

        if polymarket_price:
            return polymarket_price

        # Если RTDS не вернул цену - это ошибка
        logger.error(f"❌ Polymarket RTDS не вернул цену для {symbol}! Проверьте подключение.")
        return None

    def get_polymarket_price(self, symbol: str) -> Optional[float]:
        """
        Получить цену с Polymarket для указанного символа
        (BTCUSDT, ETHUSDT или SOLUSDT).

        Теперь активировано для использования вместо Binance.
        """
        return self.polymarket.get_price(symbol)
    ...
    async def get_price_at_time(
        ...
        """
        Получить цену close для указанного времени из Binance.
        """
        return await self.binance.get_price_at_time(symbol, timestamp)

    async def get_chainlink_price_at_time(
        self,
        symbol: str,
        timestamp: datetime,
    ) -> Optional[float]:
        """
        Получить историческую цену из Chainlink на указанный timestamp.

        Используем только локальный коллектор цен. Без fallback на historical API.
        ...
        try:
            ...
            # Получаем цену из локального коллектора
            collector_price = self.chainlink_collector.get_price_at_time(symbol, ts)
            if collector_price:
                logger.debug("Цена из локального коллектора для %s на %s: $%.2f",
                           symbol, timestamp.isoformat(), collector_price)
                return collector_price
            else:
                logger.debug("Цена %s не найдена в коллекторе на %s",
                           symbol, timestamp.isoformat())
                return None

        except Exception as exc:
            logger.error("Ошибка при получении Chainlink цены для %s: %s", symbol, exc)
            return None

    def _symbol_to_chainlink(self, symbol: str) -> Optional[str]:
        """
        Конвертировать символ в Chainlink формат.

        Возвращает символ в формате, ожидаемом ChainlinkHistoricalConnector.
        """
        # ChainlinkHistoricalConnector ожидает BTCUSDT, ETHUSDT, SOLUSDT
        mapping = {
            "BTCUSDT": "BTCUSDT",
            "ETHUSDT": "ETHUSDT",
            "SOLUSDT": "SOLUSDT",
