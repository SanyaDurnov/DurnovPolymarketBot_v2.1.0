"""
Minimal main script для тестирования Web UI.
"""
...
def web_mode_minimal():
    """Минимальный режим Web UI: запуск FastAPI сервера без шаблонов."""
    logger.info("=" * 60)
    logger.info("ЗАПУСК МИНИМАЛЬНОГО WEB UI")
    logger.info("=" * 60)

    try:
        import uvicorn
        from web.app_minimal import app
...
def main():
    """Главная функция."""
    parser = argparse.ArgumentParser(description="Polymarket Bot V2 - Minimal")
    parser.add_argument("--mode", choices=["web"], default="web", help="Режим работы")

    args = parser.parse_args()

    logger.info("Polymarket Bot V2 - Minimal Mode")
    logger.info("Режим: %s", args.mode)
    logger.info("")

    try:
        if args.mode == "web":
            return web_mode_minimal()
        else:
