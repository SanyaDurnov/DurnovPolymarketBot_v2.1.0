...
from app.config import (
    ...
    VOLATILITY_FLOOR,
    PROBABILITY_LOG_INTERVAL_SECONDS,
    PROBABILITY_LOG_VOLATILITY,
)
...
class PostEntryProbabilityAnalyzer:
    ...
    def __init__(self, rfr: float = 0.0):
        ...
        self._last_reach_log_ts = 0.0  # Для контроля интервала REACH CALC логов
    ...
    def _calculate_target_probabilities(
        ...
        """
        Вспомогательный метод для расчета вероятностей достижения цели.
        
        Returns:
            (reach_prob_raw, reach_prob_up, reach_prob_down, price_move_pct, std_dev_pct, z_score, time_in_hours)
        """
        ...
        return reach_prob_raw, reach_prob_up, reach_prob_down, price_move_pct, std_dev_pct, z_score, time_in_hours
    ...
    def calculate_probabilities(
        ...
        """
        Рассчитывает вероятности для UP и DOWN целей.
        
        Args:
            current_price: Текущая цена
            target_up: Цель для UP (например, 2200)
            target_down: Цель для DOWN (например, 2000)
            volatility_pct: Волатильность (например, 0.06 для 6%)
            time_remaining_minutes: Время до экспирации в минутах
            vol_period_minutes: Период волатильности (15 или 60)
            
        Returns:
            dict: {
                'p_up': float,      # Вероятность быть выше UP цели
                'p_down': float,    # Вероятность быть ниже DOWN цели
            }
        """
        # Рассчитываем гибридную волатильность
        vol_for_kf, atr_pct, vol_current = self.calculate_hybrid_volatility(
            volatility_pct, None, current_price
        )
        
        # Рассчитываем p_up (вероятность быть выше UP цели)
        price_move_pct_up = (target_up - current_price) / current_price
        std_dev_pct = vol_for_kf * math.sqrt(time_remaining_minutes / vol_period_minutes)
        z_score_up = price_move_pct_up / std_dev_pct
        p_up = 1.0 - self.calculate_normal_cdf(z_score_up)
        
        # Рассчитываем p_down (вероятность быть ниже DOWN цели)
        price_move_pct_down = (target_down - current_price) / current_price
        z_score_down = price_move_pct_down / std_dev_pct
        p_down = self.calculate_normal_cdf(z_score_down)
    ...
    def calculate_probabilities_for_position_manager(
        self,
        current_price: float,
        target_price: float,
        volatility_pct: float,
        ...
        atr: float | None = None,
        vol_period_minutes: float = 15.0,
    ) -> dict:
        """
        Рассчитывает вероятности для position_manager.
        Использует существующий интерфейс analyze_opposite_side_buy.
        
        Args:
            current_price: Текущая цена актива
            target_price: Целевая цена (price_to_beat)
            volatility_pct: Волатильность (уже рассчитана)
            time_remaining_minutes: Время до экспирации в минутах
            market_direction: Направление рынка ("UP" или "DOWN")
            current_price_up: Текущая цена UP исхода
            current_price_down: Текущая цена DOWN исхода
            atr: ATR значение
            vol_period_minutes: Период волатильности (15 или 60)
            
        Returns:
            dict: {
                'p_up': float,      # Вероятность быть выше target
                'p_down': float,    # Вероятность быть ниже target
                'volatility_used': float,  # Использованная волатильность
                'atr_used': float | None,  # Использованный ATR
                'market_direction': str,   # Направление рынка
                'edge_up': float,   # Edge для UP
                'edge_down': float, # Edge для DOWN
            }
        """
        # Лог входных данных
        ...
        print(f"    volatility_pct = {volatility_pct*100:.2f}%")
        print(f"    time_remaining_minutes = {time_remaining_minutes}")
        print(f"    market_direction = {market_direction}")
        print(f"    current_price_up = {current_price_up}")
        print(f"    current_price_down = {current_price_down}")
        print(f"    atr = {atr}")
        print(f"    vol_period_minutes = {vol_period_minutes}")
        
        # Рассчитываем вероятности
        prob_up_result = self.analyze_opposite_side_buy(
            current_price_btc=current_price,
            market_target=target_price,
        ...
        prob_down_result = self.analyze_opposite_side_buy(
            current_price_btc=current_price,
            market_target=target_price,
            market_direction=market_direction,
            opposite_side="DOWN",
        ...
        # Лог результатов
        print(f"  Результаты:")
        print(f"    p_up = {prob_up_result.math_probability*100:.3f}%")
        print(f"    p_down = {prob_down_result.math_probability*100:.3f}%")
        print(f"    edge_up = {prob_up_result.edge*100:.3f}%")
        print(f"    edge_down = {prob_down_result.edge*100:.3f}%")
        
        return {
            'p_up': prob_up_result.math_probability,
            'p_down': prob_down_result.math_probability,
            'volatility_used': volatility_pct,
            'atr_used': atr,
            'market_direction': market_direction,
            'edge_up': prob_up_result.edge,
            'edge_down': prob_down_result.edge,
        }
    ...
    def analyze_opposite_side_buy(
        ...
        # Рассчитываем вероятности достижения цели
        (reach_prob_raw, reach_prob_up, reach_prob_down, 
         price_move_pct, std_dev_pct, z_score, time_in_hours) = self._calculate_target_probabilities(
            current_price_btc,
            market_target,
            vol_for_kf,
            time_remaining_minutes,
        ...
        # Определяем математическую вероятность для противоположной стороны
        if opposite_side == "UP":
        ...
        else:  # opposite_side == "DOWN"
            math_prob_opposite = reach_prob_down
            direction_note = f"{opposite_side} (market_direction={market_direction})"
            inverted = (market_direction == "UP")

        # Логируем расчеты с интервальным контролем
        now = time.time()
        should_log_reach = now - self._last_reach_log_ts >= PROBABILITY_LOG_INTERVAL_SECONDS
        
        if should_log_reach:
            self._last_reach_log_ts = now
            
            # Формируем лог с волатильностью
            log_message = (
                f"REACH CALC: current_price={current_price_btc:.2f}, target={market_target:.2f}, "
                f"reach_prob_raw={reach_prob_raw:.4f}, price_move_pct={price_move_pct:.4f}, "
                f"z_score={z_score:.4f}, reach_prob_up={reach_prob_up:.4f}, "
                f"reach_prob_down={reach_prob_down:.4f}, math_prob_opposite={math_prob_opposite:.4f}"
            )
            
            # Добавляем волатильность в лог, если включено
            if PROBABILITY_LOG_VOLATILITY:
                log_message += f" | VOL: hybrid={vol_for_kf*100:.2f}%, atr={atr_pct*100:.2f}%, current={vol_current*100:.2f}%"
            
            logger.info(log_message)

        # Определяем рыночную цену для противоположной стороны
        market_price_opposite = current_price_up if opposite_side == "UP" else current_price_down
        market_prob_opposite = market_price_opposite

        # Рассчитываем edge и KF
        ...
        logger.debug(
            ...
            direction_note,
            reach_prob_up * 100 if opposite_side == "UP" else reach_prob_down * 100,
        ...
        return ProbabilityCoefficient(
            ...
            z_score=z_score,
            reach_probability=reach_prob_up if opposite_side == "UP" else reach_prob_down,
            inverted=inverted,
        )
