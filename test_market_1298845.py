#!/usr/bin/env python3
"""
Test Price to beat for market 1298845.
"""
...
def main():
    print('🧪 Тестирование Price to beat для рынка 1298845...')
    ...
    print(f'📋 Вопрос: {market_data.get("question", "")[:80]}...')
    print(f'📊 Активен: {market_data.get("active")}')
    print(f'❌ Закрыт: {market_data.get("closed")}')
    ...
    print(f'🕐 Прошло времени: {hours_diff:.1f} часов')

    if hours_diff < 0:
        print('⏰ Рынок еще не открылся!')
    elif hours_diff < 1:
        print('🆕 Рынок открылся недавно')
    else:
    ...
    # Тестируем API
    print('🌐 Тестируем API /api/market/1298845...')
    import subprocess
    try:
