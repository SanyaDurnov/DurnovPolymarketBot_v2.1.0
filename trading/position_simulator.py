...
from app.config import SIMULATION_MAX_POSITION_PCT, SIMULATION_POSITION_SIZE_PCT
from app.web_state import stats_store
...
class SimulatedPosition:
    ...
    market_duration_minutes: float | None = None
...
class PositionSimulator:
    ...
    def open_position(
        ...
        self._record_event(
            "entry",
            f"Position {position_id} opened ({decision})",
            {
                "cost": position.entry_cost,
                "side": position.side or "BOTH",
            },
        )
    ...
    def add_to_position(
        ...
        max_position_usd = self.current_balance * SIMULATION_MAX_POSITION_PCT
        if position.entry_cost + amount_usd > max_position_usd:
            logger.warning(
                "[SIM] Add blocked: max position size $%.2f exceeded (current=$%.2f, add=$%.2f)",
                max_position_usd,
                position.entry_cost,
                amount_usd,
            )
        ...
        self._record_event(
            "add",
            f"Position {position_id} ADD {side}",
            {"amount": amount_usd, "side": side},
        )
        return True
    ...
    def add_to_position(
        ...
        if amount_usdc > self.current_balance:
            ...
            return False
        max_position_usd = self.current_balance * SIMULATION_MAX_POSITION_PCT
        if position.entry_cost + amount_usdc > max_position_usd:
            logger.warning(
                "[SIM] Add blocked: max position size $%.2f exceeded (current=$%.2f, add=$%.2f)",
                max_position_usd,
                position.entry_cost,
        ...
        self._record_event(
            "add",
            f"Position {position.position_id} ADD {side}",
            {"amount": amount_usdc, "side": side},
        )
        return True
    ...
    def close_position(
        ...
        # Не обнуляем выручку: закрываем по текущим бид-ценам.
    ...
    def _record_event(
        ...
        stats_store.add_event(payload)
        self._refresh_stats(payload)

    def _refresh_stats(self, payload: dict[str, Any]) -> None:
            ...
            open_positions.append(
                {
                    ...
                    "market_duration_minutes": position.market_duration_minutes,
                }
            )
        closed_positions = []
            ...
            closed_positions.append(
                {
                    ...
                    "entry_cost": position.entry_cost,
                    "market_duration_minutes": position.market_duration_minutes,
                    "exit_revenue": position.exit_revenue,
                    "pnl": position.pnl,
        ...
        stats_store.update(
            ...
            closed_positions_count=summary["closed_positions"],
            last_trade=payload,
