...
class BinanceConnector:
    ...
    def get_last_price(self, symbol: str) -> Optional[float]:
        ...
        Сначала проверяем кэш из WebSocket, если не запущен - используем REST API.
        ...
        """
        # Сначала проверяем кэш из WebSocket
        cached_price = self.last_prices.get(symbol)
        if cached_price is not None:
            return cached_price

        # Если WebSocket не запущен или нет данных, используем REST API
        try:
            # Создаем синхронный клиент для быстрого запроса
            from binance.client import Client
            client = Client(api_key=self.api_key or "", api_secret=self.api_secret or "")

            # Получаем текущий тикер (включает последнюю цену)
            ...
            logger.debug("Не удалось получить цену через REST API для %s: %s", symbol, exc)
    ...
    async def get_price_at_time(
        ...
        try:
            ...
            logger.error("Ошибка Binance API при получении цены по времени: %s", exc)
            return None
        except Exception as exc:  # noqa: BLE001
            logger.error(
                "Ошибка при получении цены по времени: %s", exc, exc_info=True
            )
            return None
